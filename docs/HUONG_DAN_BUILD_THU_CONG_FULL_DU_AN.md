...
> 💡 **Cơ chế:** Lệnh `--recursive` sẽ tự động tải `agentcore-studio-kit` và tự động kéo toàn bộ 8 repo con (`studio-app`, `workbench`, `engine`, `kb`, `evalhub`, `web`, `contracts`, `report`) đặt vào đúng vị trí trong `VSF/Build`.
...
# 2. Clone đầy đủ 9 Repositories từ GitHub (gồm cả studio-app)
git clone https://github.com/AI20K-VGR/agentcore-studio-kit.git
git clone https://github.com/AI20K-VGR/agentcore-studio-app.git
...
Copy-Item -Path ".\agentcore-studio-app\*" -Destination ".\VSF\Build\apps\studio\" -Recurse -Force
...
## 🔗 III. KÍCH HOẠT VÀ LIÊN KẾT CÁC GÓI (BẮT BUỘC ĐỦ 6 PACKAGES)

Sau khi đã có thư mục `VSF/Build` (theo Cách 1 hoặc Cách 2), di chuyển vào `VSF/Build` để liên kết tất cả 6 packages bao gồm cả `apps/studio`:

```powershell
cd .\VSF\Build\
pip install -e packages/contracts -e packages/workbench -e packages/engine -e packages/kb -e packages/evalhub -e apps/studio
```
