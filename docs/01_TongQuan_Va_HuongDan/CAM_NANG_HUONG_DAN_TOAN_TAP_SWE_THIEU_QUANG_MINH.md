...
Bạn là người **"Giữ Bút"** cho **Contract #1: Recipe Schema** tại file `packages/contracts/src/studio_contracts/recipe.py`.
...
Tại thư mục `packages/workbench/src/studio_workbench/`, bạn cần hiện thực 4 file sau:
...
* 🧪 **Bài test tương ứng để kiểm tra:** `packages/workbench/tests/test_graph_lint.py`. Khi bạn code đúng 4 luật này, bài test sẽ từ màu đỏ (`xfail`) chuyển sang xanh lá (`PASS`).

---

### 📄 File 2: `tenant_wall.py`
...
Thư mục Web đặt tại `apps/web/`. Công nghệ sử dụng: **React 19 + TypeScript + Vite + React Flow**.

Bạn có nhiệm vụ phát triển giao diện người dùng từ file khung `apps/web/src/App.tsx` thành 3 màn hình/chức năng chính:
...
1. Mở file `packages/workbench/src/studio_workbench/validator.py` và cài đặt 4 luật của hàm `graph_lint()`.
2. Mở file `packages/workbench/tests/test_graph_lint.py` và chạy `uv run pytest` để xác nhận test chuyển sang màu xanh (`PASS`).
3. Tiếp tục hoàn thiện `tenant_wall.py` và `publish.py`.
4. Dựng Form và Canvas React Flow trên `apps/web/src/App.tsx`.
